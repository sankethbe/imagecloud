...
error_log('inside instagraph.php');
...
class instagraph
{
  ...
  {error_log('inside setInput');
    if (file_exists($path))
    {
	error_log('instagraph input path exists');
      $this->_input = $path;
  ...
  {error_log('inside resize');
    $this->execute("convert $this->_input -resize {$w}x{$h} -unsharp 1.5Ã—1.0+1.5+0.02 $this->_input");
  }
